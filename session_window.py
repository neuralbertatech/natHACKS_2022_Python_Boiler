...
class session_win(QWidget):
    ...
    def find_phase(self,df):
        ...
        inst_freq_series = (np.diff(np.unwrap(inst_phase_series))/(2.0*np.pi) * self.sampling_rate) 
        ...
        pred_ground_truth_x = np.arange(0,np.ceil(predict_len)+1)*(1/self.sampling_rate)+x[50]
        ...
        pred_new_x = np.arange(0,np.ceil(predict_len))*(1/self.sampling_rate)+pred_ground_truth_x[-1]

...
class PlusMinusButton:
    ...
    def draw(self, surface):
        ...
        pg.draw.line(
            surface,
            BLACK,
            (self.x + self.button_w, self.y),
            (self.x + self.button_w, self.y + self.h),
            3,
        )
        pg.draw.line(
            surface,
            BLACK,
            (self.x + self.w - self.button_w, self.y),
            (self.x + self.w - self.button_w, self.y + self.h),
            3,
        )
        ...
        for ev in ev_list:
            if ev.type == pg.MOUSEBUTTONDOWN:
                if self.plus_box.collidepoint(mouse) and self.content < 1:
                    self.content = round(
                        self.content + 0.1, 1
                    )  # Rounding helps w/ floating-point errors

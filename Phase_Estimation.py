...
col_names = ['chan_1','chan_2','chan_3','chan_4','chan_5','chan_6','chan_7','chan_8','chan_9','chan_10','chan_11','chan_12','chan_13','chan_14','chan_15','chan_16','trig']
...
for chan in range(chan_num):
    DataFilter.perform_lowpass(data[chan], sampling_rate, butter_cutoff, butter_order, FilterTypes.BUTTERWORTH.value, 0)
...
for targ in range(len(targ_trigs)):
    for chan in range(chan_num):  
        ...
        for intra_epoch in range(intra_epoch_num): # range(len(intra_epoch_ind) 
            ...
            psd = DataFilter.get_psd_welch(df.iloc[targ_win_low:targ_win_high,chan].to_numpy(), nfft, nfft // 2, sampling_rate, WindowFunctions.BLACKMAN_HARRIS.value)
...
DataFilter.perform_bandpass(Hjorth, sampling_rate, bandpass_center, bandpass_width, butter_order, FilterTypes.BUTTERWORTH.value, 1)
...
avg_inst_freq = np.mean(inst_freq_series[int(len(inst_freq_series)*0.2):-np.int(len(inst_freq_series)*.2)])
print(avg_inst_freq)
...
phase_diff = 2*np.pi - np.abs(desired_phase - inst_phase_series[-1]) - np.pi/2 # phase goes from 0 at 2pi and goes down to
...
plt.plot(tech_delay_x,tech_delay_y,linewidth=3, label="technical delay")
plt.plot(x[int(len(phase_target)/2):-1],phase_target[int(len(phase_target)/2):-1],linewidth=2, label="alpha carrier")
plt.plot(x[int(len(inst_phase_series)/2+len(full_wait_x)):-1],inst_phase_series[int(len(inst_phase_series)/2+len(full_wait_x)):-1],linewidth=2, label="phase series")
...
pred_groud_truth_y = ar_model.predict(start=len(train_data), end=phase_len, dynamic=False)
...
# Make the predictions
pred_new_y = ar_model.predict(start=len(train_data), end=int(len(train_data)-1+predict_len), dynamic=False)
...
# Make the predictions
pred_new_y = ar_model.predict(start=len(train_data)+1, end=len(train_data)+predict_len, dynamic=False)
